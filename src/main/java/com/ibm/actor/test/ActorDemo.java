...
/**
 * A GUI for running and visualizing Actors. The implementation of this GUI is not discussed in this associated article.
 * 
 * @author BFEIGENB
 *
 */
public class ActorDemo extends JPanel implements ChangeListener {
    ...
    static {
        /*
         * for (int r = 0; r < 3; r++) { for (int g = 0; g < 3; g++) { for (int b = 0; b < 3; b++) { // don't get too
         * close to white Color c = new Color(128 + rand.nextInt(100), 128 + rand.nextInt(100), 128 +
         * rand.nextInt(100)); if (!colors.contains(c)) { colors.add(c); } } } }
         */
        int r = 0, g = 0, b = 0;
        for (int i = 0; colors.size() < 64 && i < 1000; i++) {
            Color c = new Color(Math.min(128, (150 + r) % 256), Math.min(128, (128 + g) % 256),
    ...
    protected void buildTopPanel(JPanel inputs) {
        ...
        sendRateBar.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                ...
                sendRateBar
                        .setString(String.format(value > maxSends - 1 ? (">=" + maxSends + " mps") : "%d mps", value));
            }
        });
        ...
        dispatchRateBar.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                ...
                dispatchRateBar.setString(
                        String.format(value > maxThreads - 1 ? (">=" + maxThreads + " cps") : "%d dps", value));
            }
        });
    ...
    protected void buildBottomPanel(JPanel views) {
        ...
        for (int i = 0; i < subActorViewCount; i++) {
            ...
            subActorViews[i] = av;
            av.setPreferredSize(
                    new Dimension((VIEWPORT_WIDTH - subActorViewCount * 5) / subActorViewCount, VIEWPORT_HEIGHT / 5));
            gp.add(av);
        }
                ...
                try {
                    ...
                    if (fx >= 0 && fy >= 0) {
                        ...
                        if (tx >= 0 && ty >= 0) {
                            ...
                            int xx = fx + (int) (dx * 0.85) - markerWidth / 2,
                                    xy = fy + (int) (dy * 0.85) - markerWidth / 2;
            ...
            if (ar != null) {
                ...
                if (ar.hasThread && a != null) {
                    ...
                    if (count != null) {
                        ...
                        g2d.drawString(Integer.toString(count), x - tdiam / 2,
                                y + (y > height / 2 ? tdiam + 10 : -tdiam - 10));
                    }
                }
            }
        ...
        try {
            ...
            if (x >= 0 && y >= 0) {
                ...
                if (selfSends.contains(aa)) {
                    ...
                    Color c = isTransparentActors
                            ? new Color(xacolor.getRed(), xacolor.getGreen(), xacolor.getBlue(), 150)
                            : xacolor;
                    g2d.setColor(c);
    ...
    protected class UpdateRunnable implements Runnable {
        ...
        protected void renderSnapshots() {
            ...
            BufferedImage bi = new BufferedImage(avLast.getWidth(), avLast.getHeight(), BufferedImage.TYPE_INT_ARGB);
            Graphics g = bi.getGraphics();
            renderTo(g, bi.getWidth(), bi.getHeight());
            addTimeStamp(-5, bi);
            avLast.setImage(bi);
                ...
                public void run() {
                    ...
                    if (actorManager != null) {
                        ...
                        if (trendLineView != null) {
                            trendLineView.setImage(
                                    renderTrendLine(actorManager.getTrendValue(), actorManager.getMaxTrendValue()));
                        }
                    }
                }
            });
